...
function Dropdown({ item }) {
  console.log(item.product_detail?.variants, "variants");
    ...
    <div className="relative font-[sans-serif] w-max mx-auto">
      ...
      >
        {/* {selectedVariant?.length > 0
          ?`${selectedVariant?.inventory?.weight} ${selectedVariant?.inventory?.unit}`
          : 
          `${item?.product_detail?.variants[0]?.inventory?.weight} ${item?.product_detail?.variants[0]?.inventory?.unit}`
        } */}
       {selectedVariant?.inventory?.weight} {selectedVariant?.inventory?.unit}
      ...
      {isOpen && (
        ...
        >
          {item.product_detail?.variants?.map((variant) => (
            <li
